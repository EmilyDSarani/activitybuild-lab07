  ...
  toxicity.load(threshold).then(model => {
    ...
    model.classify(thingsToCheck).then(predictions => {
      const violations = predictions.map(prediction => {
        return {
          label: prediction.label, //label is what type of toxicness it is. Threat, insule, sexual, ect. 
          match: prediction.results[0].match //this is a boolean of true or false. If the match is true at any point, then it will error.
        };
      });
      ...
      if (!violations.find(violation => violation.match)){
        ...
        err.status = 420;
        // await User.newOffense()
        //in the model, whenever I call newOffense, it just adds 1 in some offence column in the user table
        next(err);
      }
    }); 
  });
